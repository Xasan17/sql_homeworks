...
--4
SELECT COUNT(DISTINCT Category) AS Category_unique FROM Products;
--5
...
--7
SELECT DepartmentName, COUNT(*) AS Count_Emp FROM Employees
...
HAVING COUNT(*)>5;
--11
SELECT SUM(SaleAmount) AS Total, AVG(SaleAmount) AS Avg FROM Sales
GROUP BY ProductID;
--12
SELECT COUNT(EmployeeID) AS Count FROM Employees
WHERE DepartmentName = 'HR'; 
--13
SELECT DepartmentName, MIN(Salary) AS Min_Dept, MAX(Salary) AS Max_Dept FROM Employees
GROUP BY DepartmentName;
--14
SELECT DepartmentName, AVG(Salary) AS Avg_Dept FROM Employees
GROUP BY DepartmentName;
--15
SELECT DepartmentName, AVG(Salary) AS Avg_Dept, COUNT(*) AS Count_Dept FROM Employees
GROUP BY DepartmentName;
--16
...
--20
SELECT ProductID, AVG(SaleAmount) AS Avg_sales FROM Sales
GROUP BY ProductID
...
--23
SELECT CustomerID, MIN(TotalAmount) AS Min, MAX(TotalAmount) AS Max FROM Orders
GROUP BY CustomerID
HAVING MIN(TotalAmount) > 50;
--24
SELECT MONTH(SaleDate) AS Month, COUNT(DISTINCT ProductID) AS Count_Unique , SUM(SaleAmount) AS Total_Sales FROM Sales
GROUP BY MONTH(SaleDate)
HAVING COUNT(DISTINCT ProductID)>8
--25
SELECT YEAR(OrderDate) AS Order_Year, MIN(Quantity) AS Quantity_Min, MAX(Quantity) AS Quantity_Max FROM Orders
